...
static uint8_t ide_read_register(uint8_t channel, uint8_t reg)
{
	...
	if(reg < 0x08)
		ret = ioport_in(g_ide_channels[channel].base + reg - 0x00);
	else if(reg < 0x0C)
		ret = ioport_in(g_ide_channels[channel].base + reg - 0x06);
	else if(reg < 0x0E)
		ret = ioport_in(g_ide_channels[channel].control + reg - 0x0A);
	else if(reg < 0x16)
		ret = ioport_in(g_ide_channels[channel].bm_ide + reg - 0x0E);
...
static void ide_write_register(uint8_t channel, uint8_t reg, uint8_t data)
{
	...
	if(reg < 0x08)
		 ioport_out(g_ide_channels[channel].base + reg - 0x00, data);
	else if(reg < 0x0C)
		ioport_out(g_ide_channels[channel].base + reg - 0x06, data);
	else if(reg < 0x0E)
		ioport_out(g_ide_channels[channel].control + reg - 0x0A, data);
	else if(reg < 0x16)
		ioport_out(g_ide_channels[channel].bm_ide + reg - 0x0E, data);
...
uint8_t ide_print_error(uint32_t drive, uint8_t err) 
{
    ...
    printf("- [");
    printf((const char *[]){"Primary", "Secondary"}[g_ide_devices[drive].channel]);
    printf(" ");
    printf((const char *[]){"Master", "Slave"}[g_ide_devices[drive].drive]);
    printf("] ");
    printf(g_ide_devices[drive].model);
    printf("\n");
            ...
            uint32_t bus_master_addr)
{
    ...
    for (i = 0; i < 4; i++)
        if (g_ide_devices[i].reserved == 1) {
            ...
            printf("\nModel: ");
            printf(g_ide_devices[i].model);
            printf("\nDrive: ");
            printf(g_ide_devices[i].drive);
            printf("\n");

            printf("[OK] ATA Driver Initialized\n");
        }
}
...
int ide_read_sectors(uint8_t drive, uint8_t num_sectors, uint32_t lba, uint32_t buffer)
{
    ...
    // 2: Check if inputs are valid:
        ...
        printf("IDE ERROR: LBA address is greater than the available drive sectors\n");
        return -2;
    }
